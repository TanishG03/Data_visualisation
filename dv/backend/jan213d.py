...
def export_visualization_data(X, H, knn_indices, title):
    ...
    'heidi_matrix': [
        [float(val) for val in row]
        for row in H_combined_reordered  # Using the ordered matrix
    ],
    'ordering': [int(i) for i in order]  # Add the ordering array
}
    
    import json
    with open(f'./visualization_data_ordered_{title.replace(" ", "_")}.json', 'w') as f:
